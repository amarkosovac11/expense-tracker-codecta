  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-sm font-semibold">Expenses Calendar</h3>
          <p className="text-xs text-muted-foreground">
            Daily expense overview for {monthLabel(currentMonth)}
          </p>
        </div>
        ...
        <div className="grid w-full grid-cols-7 gap-1">
          {calendarDays.map((date, index) => {
            ...
            return (
              <div
                key={key}
                className={`min-h-[52px] rounded-md border px-1.5 py-1 transition-all hover:shadow-sm ${getExpenseBg(
                  level
